...
export default function ESGChart({ portfolio, scenario, showComparison }) {
  ...
  const active = animated ? (compEsg || esg) : esg
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.gaugeCol}>
        ...
        <svg viewBox="0 0 320 255" style={s.gaugeSvg}>
          {/* Track */}
          ...
          {/* Base arc — always visible when comparing */}
          {compEsg && (
          ...
          {/* Base needle dot (when comparing) */}
          {compEsg && (() => {
            const bp = pt(baseAngle)
            return (
              <circle cx={bp.x} cy={bp.y} r="7"
                fill="rgba(255,255,255,0.25)" stroke="#0C182E" strokeWidth="2" />
            )
          })()}

          {/* Active needle — transform for reliable CSS animation across browsers */}
          <g transform={`translate(${needlePt.x} ${needlePt.y})`}
            style={{ transition: 'transform 0.85s cubic-bezier(0.4,0,0.2,1)' }}>
            <circle r="13" fill={scoreColor} stroke="#0C182E" strokeWidth="3"
          ...
          <text x={cx} y={cy + 24} textAnchor="middle"
            fontFamily="'Merriweather', serif" fontSize="54" fontWeight="700"
            fill={scoreColor}
            style={{ transition: 'fill 0.5s ease' }}>
          ...
          {[0,2,4,6,8,10].map(v => {
            ...
            const pt2 = { x: cx+(r+26)*Math.cos(a*Math.PI/180), y: cy+(r+26)*Math.sin(a*Math.PI/180) }
            return (
              <g key={v}>
            ...
            <div style={s.sfdrRow}>
              ...
              <div style={{ flex: 1 }}>
                <div style={s.sfdrTrack}>
                  {/* Base bar when comparing */}
                  {compEsg && esg.sfdr.find(x => x.article === item.article) && (() => {
                    const base = esg.sfdr.find(x => x.article === item.article).weight
                    return (
                      <div style={{
                        position: 'absolute', top: 0, bottom: 0, left: 0,
                        borderRadius: 4,
                        background: sfdrColors[item.article],
                        width: `${base}%`, opacity: 0.18,
                      }} />
                    )
                  })()}
                  <div style={{
                    ...
                    background: sfdrColors[item.article],
                    width: animated ? `${item.weight}%` : `${esg.sfdr.find(x => x.article === item.article)?.weight || 0}%`,
                    opacity: 0.78,
...
const s = {
  wrap: { display: 'flex', gap: 44, height: '100%', width: '100%', alignItems: 'center' },
  gaugeCol: { flexShrink: 0, width: '42%', maxWidth: 340, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0, height: '100%', justifyContent: 'center' },
  gaugeSvg: { width: '100%', height: 'auto', maxHeight: '240px' },
  sfdrCol: { flex: 1, display: 'flex', flexDirection: 'column', gap: 16, justifyContent: 'center' },
  label: { fontFamily: "'Merriweather Sans', sans-serif", fontSize: '0.58rem', fontWeight: 800, color: 'rgba(255,255,255,0.28)', letterSpacing: '0.1em', marginBottom: 4 },
  metrics: { display: 'flex', gap: 10, marginTop: 4 },
  metricCard: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '10px 16px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8 },
  mLabel: { fontFamily: "'Merriweather Sans', sans-serif", fontSize: '0.62rem', color: 'rgba(255,255,255,0.38)' },
  mVal: { fontFamily: "'Merriweather', serif", fontSize: '1.7rem', fontWeight: 700 },
