...
function heatColor(weight, max) {
  ...
  if (t < 0.33) return `rgba(94,163,255,${0.3 + t * 0.8})`
  if (t < 0.66) return `rgba(78,213,150,${0.35 + t * 0.7})`
  return `rgba(224,27,65,${0.4 + (t - 0.66) * 0.9})`
}
      ...
      <div style={s.matrixCol}>
              ...
              {STYLES.map(style => {
                ...
                return (
                  ...
                  >
                    ...
                    {hasChange && (
                      <span style={{ ...s.cellDelta, color: delta > 0 ? '#E01B41' : '#4ED596' }}>
                        {delta > 0 ? '+' : ''}{delta}
                      </span>
                    )}
                  </div>
        ...
        <div style={s.tiltRow}>
          <div style={s.tiltCard}>
            ...
            <div style={{
              ...s.tiltValue,
              color: growthTilt > 5 ? '#E01B41' : growthTilt < -5 ? '#4ED596' : 'rgba(255,255,255,0.7)',
            }}>
              {Math.abs(growthTilt) < 5 ? 'Balanced'
                : growthTilt > 0 ? `Growth +${growthTilt}%`
                : `Value +${Math.abs(growthTilt)}%`}
            </div>
          </div>
          <div style={s.tiltCard}>
            <div style={s.tiltLabel}>LARGE CAP</div>
            <div style={s.tiltValue}>{largeTilt}%</div>
          </div>
        </div>
      </div>
            ...
            <div key={style} style={s.barRow}>
              ...
              <div style={s.track}>
                <div style={{
                  ...
                  background: style === 'Value' ? '#5B8DEF' : style === 'Blend' ? '#4ED596' : '#E01B41',
            ...
            <div key={cap} style={s.barRow}>
              ...
              <div style={s.track}>
                <div style={{
                  ...
                  background: cap === 'Large' ? '#F5A623' : cap === 'Mid' ? '#A78BFA' : '#8A8A82',
                  transition: 'width 0.8s cubic-bezier(0.4,0,0.2,1)',
                }} />
              </div>
...
const s = {
  ...
  cellVal: { fontFamily: "'Merriweather Sans', sans-serif", fontSize: '0.88rem', fontWeight: 800, color: 'rgba(255,255,255,0.9)' },
  ...
  tiltValue: { fontFamily: "'Merriweather Sans', sans-serif", fontSize: '0.88rem', fontWeight: 800, color: 'rgba(255,255,255,0.88)' },
  ...
  barLabel: { fontFamily: "'Merriweather Sans', sans-serif", fontSize: '0.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.7)', width: 72, flexShrink: 0 },
